...
def extract_features(image):
    ...
    for r in results:
        for box in r.boxes:
            ...
            if any(
                v in label for v in ["car", "truck", "bus", "motorcycle", "vehicle"]
            ):
                num_vehicles += 1
            elif any(
                k in label
                ...
                k in label for k in ["tree", "pole", "post", "wall", "barrier", "rock"]
            ):
...
async def predict(request: Request, file: UploadFile = File(...)):
    ...
    # YOLO accident detection (disable auto-save)
    results = accident_model.predict(
        temp_file, conf=CONF_THRESHOLD, save=False, device="cpu"  # VERY IMPORTANT
    )
    ...
    for r in results:
                    ...
                    detections.append(
                        {
                            ...
                            "label": (
                                ...
                                if hasattr(accident_model, "names")
                                else str(class_id)
                            ),
        ...
        save_success = cv2.imwrite(annotated_path, annotated_image)

        if not save_success:
            print("❌ Failed to save annotated image:", annotated_path)
    ...
    if (
        damage_area > 0.45
        or num_vehicles >= 3
        or (collision_object and damage_area > 0.25)
    ):
        rule_label = "severe"
    ...
    annotated_url = f"{base_url}/annotated/{unique_filename}"

    print("DEBUG Annotated URL:", annotated_url)
